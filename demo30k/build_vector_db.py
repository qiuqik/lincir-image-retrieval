...
class Flickr30kDataset:
    def __init__(self, dataset_path: Path, preprocess):
        ...
        # Flickr30k images are typically stored directly in the folder passed
        self.images_dir = self.dataset_path
        self.names = sorted([p.stem for p in self.images_dir.iterdir() if p.suffix.lower() in ('.jpg', '.jpeg', '.png', '.webp')])
        self.preprocess = preprocess
        self.split = 'flickr30k'
    ...
    def __getitem__(self, idx):
        ...
        raise FileNotFoundError(f"Image not found for stem {name} in {self.images_dir}")
...
def build_index(dataset_path: str, out_dir: str, clip_model_name: str = 'large', cache_dir: str = './hf_models', batch_size: int = 32, num_workers: int = 4):
    ...
    dataset = Flickr30kDataset(Path(dataset_path), clip_preprocess)
    ...
    out_dir_path = Path(out_dir)
    out_dir_path.mkdir(parents=True, exist_ok=True)
    faiss.write_index(index, str(out_dir_path / 'flickr30k.index'))
    with open(out_dir_path / 'names.pkl', 'wb') as f:
        pickle.dump(names, f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset-path', default='./datasets/Flickr30k/flickr30k-images', help='Path to Flickr30k images folder')
    parser.add_argument('--out-dir', default='./experiment/flickr30k_index', help='Output directory to save index and names')
